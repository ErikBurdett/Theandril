...
describe('preserved schema-4 archives and mixed-version continuation', () => {
  it('matches the independently recorded pre-travel short-campaign v4 benchmark seal', () => {
    ...
    const state = createGame({ seed: 20260905, size: 'tiny', factionCount: 4, pace: 'short', generatorVersion: 1, rosterVersion: 1, rulesVersion: 4 });
  ...
  it('migrates actual v4 snapshot structure without rewriting its bytes, events or checkpoint seals', () => {
    const state = createGame({ seed: 74, size: 'tiny', factionCount: 2, pace: 'short', generatorVersion: 1, rosterVersion: 1, rulesVersion: 4 });
    const prior = legacyOrigin(state);
  ...
  it('preserves original victory seals and postgame records after importing an old completed campaign', () => {
    const state = prosperityCampaign(4); state.rosterVersion = 1; state.world.generatorVersion = 1; state.world.biome = deriveBiomes(state.world.seed, state.world.width, state.world.height, state.world.terrain, 1);
  ...
  it('rejects invented versions, lost format metadata and a downgrade after modern records', () => {
    const state = createGame({ seed: 74, size: 'tiny', factionCount: 2, pace: 'short', generatorVersion: 1, rosterVersion: 1, rulesVersion: 4 }); const prior = legacyOrigin(state);
