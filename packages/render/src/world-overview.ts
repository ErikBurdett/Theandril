 ...
 */
export function worldOverviewPixels(width: number, height: number, cells: Iterable<MapObservation['cells'][number]>) {
  ...
  for (const cell of cells) {
    ...
    const color = isLake(cell.hydrology ?? 0) ? 0x59969d : cell.terrain === 0 ? cell.waterDepth === 2 ? 0x1a303e : 0x3a6266 : COLORS[cell.biome] ?? COLORS[1]!;
    const alpha = cell.visible ? 1 : .4;
    for (let dy = 0; dy < 2; dy++) for (let dx = 0; dx < 2; dx++) {
      const offset = ((y + dy) * pixelWidth + x + dx) * 4;
      const value = riverSize(cell.hydrology ?? 0) && dx === 1 ? 0x9bc8ca : color;
