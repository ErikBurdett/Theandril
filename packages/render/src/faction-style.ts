...
export interface EntityArtSelection {
  ...
  presentation: 'faction' | 'generic' | 'strategic' | 'procedural';
  warning: string | null;
}
...
export function selectEntityArt(role: string, definitionId: string | undefined, far: boolean, available: (id: string) => boolean): EntityArtSelection {
  ...
  const displayedRole = far ? role.startsWith('settlement.') ? 'ui.banner' : 'ui.badge' : role;
  const requestedId = definitionId ? factionArtId(displayedRole, definitionId) : undefined;
  if (requestedId && available(requestedId)) return { requestedId, contentId: requestedId, presentation: far ? 'strategic' : 'faction', warning: null };
  const contentId = !far && available(role) ? role : null;
