...
import { BIOME, deriveBiomes, isPassable, isValidBiome, neighbors, SeededRandom } from '@theandril/mapgen';
...
import { battleStateSchema } from './combat';
...
import { armyMovement, armySight } from './army-composition';
import { LEGACY_UNIT_IDS, type RulesVersion } from './rules';
import { characterAftermathSchema, characterBattleSnapshotSchema, characterLeadership, characterSchema, rebuildCharacterIndexes, validateCharacters } from './characters';
...
const worldSchema = worldV4Schema.extend({ biome: z.array(z.number().int().refine(isValidBiome, 'Unknown biome')).max(350_000), generatorVersion: z.union([z.literal(1), z.literal(2)]) }).strict();
...
const armySchema = z.object({ id, factionId: id, name, cell, movement: integer, formations: z.array(armyFormationSchema).min(1).max(12) }).strict();
...
export const campaignBattleSchema = schema6CampaignBattleSchema.extend({
  rulesVersion: z.union([z.literal(5), z.literal(6), z.literal(7)]),
  characterSnapshots: z.array(characterBattleSnapshotSchema).max(39),
...
const stateV5Schema = stateV4Schema.extend({ world: worldSchema, routes: z.array(movementRouteSchema).max(60_000) }).strict();
const stateV6Schema = stateV5Schema.extend({ armies: z.array(armySchema).max(60_000), battle: schema6CampaignBattleSchema.nullable(), battleReports: z.array(schema6CampaignBattleSchema).max(MAX_BATTLE_REPORTS) }).strict();
const stateSchema = stateV6Schema.extend({ characters: z.array(characterSchema).max(4608), battle: campaignBattleSchema.nullable(), battleReports: z.array(campaignBattleSchema).max(MAX_BATTLE_REPORTS) }).strict();
const saveSchema = z.object({ version: z.literal(7), gameVersion: z.literal('0.1.0'), contentHash: z.string(), stateChecksum: z.string().regex(/^[a-f0-9]{8}$/), state: stateSchema }).strict();
...
function canonicalPayload(state: GameState) {
  const payload = {
      ...
      world: { seed: state.world.seed, width: state.world.width, height: state.world.height, terrain: [...state.world.terrain], fertility: [...state.world.fertility], starts: [...state.world.starts], biome: [...state.world.biome], generatorVersion: state.world.generatorVersion },
...
export function battleReportForVersion(battle: CampaignBattle, version: RulesVersion) {
  if (version === 7) return campaignBattleSchema.parse(battle);
  ...
  const { characterSnapshots: _characters, characterAftermath: _charactersAftermath, usedAbilities: _abilities, ...schema6 } = battle;
...
export function serializeGameForVersion(state: GameState, version: RulesVersion): string {
  ...
  if (version === 7) return JSON.stringify({ version: SAVE_VERSION, gameVersion: '0.1.0', contentHash: CONTENT_HASH, stateChecksum: checksum(JSON.stringify(modern)), state: modern });
  ...
  const { characters: _characters, ...beforeCharacters } = modern;
...
function parseSave(raw: unknown): z.infer<typeof saveSchema> {
  ...
  const migrateV6 = (prior: z.infer<typeof saveV6Schema>): z.infer<typeof saveSchema> => {
    ...
    const state = stateSchema.parse({ ...prior.state, characters: [], battle: prior.state.battle ? migrateBattle(prior.state.battle) : null, battleReports: prior.state.battleReports.map(migrateBattle) });
    return { version: 7, gameVersion: '0.1.0', contentHash: CONTENT_HASH, stateChecksum: checksum(JSON.stringify(state)), state };
  };
...
export function deserializeGame(text: string): GameState {
  ...
  assert(data.world.terrain.length === totalCells && data.world.fertility.length === totalCells && data.world.biome.length === totalCells, 'world array dimensions disagree');
  ...
  for (const army of data.armies) {
    ...
    assert(landCell(army.cell), 'army must occupy a passable cell');
  ...
  const validateBattle = (battle: z.infer<typeof campaignBattleSchema>, pending: boolean): void => {
    ...
    assert(landCell(battle.attackerCell) && landCell(battle.defenderCell) && neighbors(battle.attackerCell, data.world.width, data.world.height).includes(battle.defenderCell), 'battle positions must be adjacent land cells');
    ...
    for (const [i, snapshot] of battle.characterSnapshots.entries()) {
      ...
      const leadership = characterLeadership({ ...snapshot, dead: false });
      const rallyRestore = definition.role === 'marshal' && !snapshot.woundedTurns ? (COMMANDER_ABILITIES.find(item => item.id === 'ability.rally')?.moraleRestore ?? 0) + (skill?.rallyBonus ?? 0) : 0;
    ...
    if (pending) {
      assert(battle.aftermath.length === 0 && battle.formationAftermath.length === 0 && battle.characterAftermath.length === 0, 'pending battle cannot have strategic aftermath');
      ...
      const stack = data.armies.filter(army => army.cell === battle.defenderCell).map(army => army.id).sort();
      ...
      for (const ending of battle.aftermath) {
        ...
        if (ending.outcome === 'destroyed') assert(ending.strength === 0 && ending.cell === null, 'destroyed army cannot have survivors or a position');
          ...
          assert(ending.strength > 0 && ending.cell !== null && landCell(ending.cell), 'invalid surviving army aftermath');
  ...
  const state: GameState = {
    ...
    world: { ...data.world, terrain: Uint8Array.from(data.world.terrain), fertility: Uint8Array.from(data.world.fertility), biome: Uint8Array.from(data.world.biome) },
