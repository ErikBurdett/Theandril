...
import { FACTIONS, FACTION_ECOLOGIES, UNITS, checksum } from '@theandril/content';
...
describe('versioned faction rosters and paid new-culture campaigns', () => {
  it('separates physical generator identity from four, six and twelve seat catalogs', () => {
    const options = { seed: 74, size: 'tiny', factionCount: 13, generatorVersion: 4 } as const;
    const four = createGame({ ...options, rosterVersion: 1 }), six = createGame({ ...options, rosterVersion: 2 }), twelve = createGame(options);
    ...
    for (const game of [four, six, twelve]) {
  ...
  it.each(FACTIONS.slice(6).map(faction => faction.id))('%s founds, works, improves, cultivates and restores through ordinary paid commands', factionDefinitionId => {
    const game = createGame({ seed: 20260905, size: 'tiny', factionCount: 12, factionDefinitionId, pace: 'short' });
