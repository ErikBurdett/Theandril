...
export function startCampaignBattle(state: GameState, factionId: string, armyId: string, targetArmyId: string): CommandResult {
  ...
  const message = `${attacker.name} engaged ${defenders.length === 1 ? defender.name : `${defenders.length} defending armies`} at hex ${defender.cell}.`;
...
function finishCampaignBattle(state: GameState, battle: CampaignBattle, events: DomainEvent[]): void {
  ...
  for (const id of participants) {
    ...
    for (const item of army.formations) {
      ...
      item.strength = result.strength; item.morale = Math.max(1, result.morale); item.fatigue = result.fatigue;
...
export function resolveCampaignBattle(state: GameState, factionId: string, order?: BattleOrder): CommandResult {
  ...
  if (battle.rulesVersion >= 7 && order !== undefined) automaticallyRally(state, battle, [side === 'attacker' ? battle.defenderFactionId : battle.attackerFactionId], events);
