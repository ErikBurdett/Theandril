...
function preferredPort(view: Observation, cells: ObservedCells, sea: SeaKnowledge) {
  ...
  const fundedHarbor = (town: Observation['settlements'][number]) => town.buildings.includes('building.harbor') || town.queue.some(order => order.itemId === 'building.harbor');
  // Construction is paid when queued. Keep that commitment even while a new,
  // more central town becomes available or an existing port is under siege.
  const funded = towns.filter(fundedHarbor).length;
  const ports = towns.filter(town => !town.occupationTurns
    && !view.visibleSiegeSettlementIds.includes(town.id) && coastalWaters(view, town.cell, cells).some(cell => isSea(cells.get(cell))));
  return ports.filter(town => !isolated || funded < 2 || fundedHarbor(town))
...
export function coastalFoundingSite(view: Observation, army: ArmyView): number | null {
  ...
  return createCoastalFoundingAssessment(view)(army);
}

function createCoastalFoundingAssessment(view: Observation, sharedCells?: ObservedCells, sharedSea?: SeaKnowledge) {
  const towns = view.settlements.filter(town => town.factionId === view.factionId);
  const diversify = Boolean(view.growth && sparseContactNeeded(view) && view.factions.length === 1);
  const routes = new Map(view.routes.map(route => [route.armyId, route]));
  const selected = diversify ? view.armies.find(army => army.factionId === view.factionId && army.canFound && !army.carrierId && !army.movementBlocker)?.id : undefined;
  const underway = diversify && view.armies.some(army => army.factionId === view.factionId && army.canFound && routes.has(army.id));
  let cells = sharedCells, sea = sharedSea;
  const sites = new Map<string, number | null>();
  return (army: ArmyView): number | null => {
    if (!army.canFound || army.carrierId || !towns.length) return null;
    // A saved land operation must not become ferry cargo. Select its successor
    // before constructing geography; waiting founders do not each scan the chart.
    if (diversify) {
...
function findCoastalFoundingSite(view: Observation, army: ArmyView, towns: Observation['settlements'], cells: ObservedCells, sea: SeaKnowledge, diversify: boolean): number | null {
  if (diversify) {
    ...
    if (shores.length === 1) {
      ...
      for (const candidate of candidates) {
        ...
        const preview = getMovementPreview(view, army.id, candidate.cell);
...
export function planNaval(view: Observation, coinBudget: number, options: NavalPlanOptions = {}): NavalPlan {
  ...
  const foundingSite = view.growth && sparseContactNeeded(view) && view.factions.length === 1 ? createCoastalFoundingAssessment(view, cells, seaKnowledge) : undefined;
  const cargoCandidates = own.filter(army => army.domain !== 'naval' && !army.carrierId && !heldArmyIds.has(army.id) && !army.movementBlocker && !siegeArmies.has(army.id)
    ...
    && !(army.canFound && foundingSite && foundingSite(army) !== null)
  ...
  if (!fleets.length) return result();

  // One O(observed cells) shoreline pass; at most256 sampled objectives and8 fleets per plan.
  let shoreFacts: { allShores: Observation['cells']; shores: Observation['cells'] } | undefined;
  const shoreline = () => {
  ...
  let homeLand: Set<number> | undefined;
  ...
  let coastalFrontier: Set<number> | undefined;
  ...
  const explore = (fleet: ArmyView) => (navigation ??= createNavigation(view)).destination(fleet, fleet.sight, claimed, undefined,
  ...
  for (const fleet of [...fleets.slice(offset), ...fleets.slice(0, offset)].slice(0, MAX_NAVAL_FLEETS)) {
    ...
    if (!passengers.length && fleet.canAttack && !fleet.commandBlocker && fleet.morale >= 40 && fleet.fatigue <= 60) {
      ...
      const target = foreign.find(enemy => {
        ...
        const preview = getMovementPreview(view, fleet.id, enemy.cell);
    ...
    if (passengers.length) {
      ...
      const viableShores = shoreline().shores.filter(shore => viableLanding(shore.cell));
    ...
    if (cargo) {
      ...
      const boarding = shoreline().shores.filter(shore => hexDistance(shore.cell, cargo.cell, view.width) <= 8).sort((a, b) => hexDistance(a.cell, cargo.cell, view.width) * 2 + hexDistance(a.cell, fleet.cell, view.width) - hexDistance(b.cell, cargo.cell, view.width) * 2 - hexDistance(b.cell, fleet.cell, view.width) || a.cell - b.cell).slice(0, 2);
