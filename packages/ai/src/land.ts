...
export function planLand(view: Observation, budget: number): { commands: GameCommand[]; reasons: string[]; coinSpent: number } {
  ...
  const entries = view.land.settlements;
  const offset = entries.length ? ((view.turn - 1) * 8) % entries.length : 0;
  const candidates = [...entries.slice(offset), ...entries.slice(0, offset)].slice(0, 8);
  ...
  for (const land of candidates) {
    ...
    if (!town || land.workPaused) continue;
    ...
    const improvement = worked.flatMap(cell => cell.improvementId ? [] : cell.improvementOptions.filter(option => option.canStart && option.coinCost <= budget).map(option => ({ cell, option, definition: IMPROVEMENTS.find(item => item.id === option.improvementId) })))
      .filter(candidate => candidate.definition).sort((a, b) => value(b.definition!.yields, needsFood) - value(a.definition!.yields, needsFood) || a.option.coinCost - b.option.coinCost || a.cell.cell - b.cell.cell);
