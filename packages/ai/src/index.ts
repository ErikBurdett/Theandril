  ...
  for (const army of rotate(ownArmies, 32).slice(0, 128)) {
    ...
    if (army.canFound && portSite !== null && portSite !== army.cell && view.growth && sparseContactNeeded(view) && view.factions.length === 1) {
      ...
      if (route?.status === 'active') {
        if (reasons.length < 16) reasons.push(`${army.id} continues its saved maritime outlet route to ${portSite}; do not replace it with local exploration.`);
        continue;
      }
      ...
      if (preview?.canQueue && preview.action === 'move') {
        ...
        reasons.push(`${army.id} retains a charted ${preview.cost}-movement route to a maritime outlet; pay founding and harbor costs on arrival.`);
      }
      else if (reasons.length < 16) reasons.push(`${army.id} waits on its maritime outlet route: ${preview?.blocker ?? 'no observed route is available'}`);
      continue;
    }
    ...
    // One representative per adjacent stack bounds public attack queries to six;
    // strengthByCell still includes every container and strategic reserve.
    const targets = adjacent.flatMap(cell => (enemyByCell.get(cell) ?? []).slice(0, 1));
    ...
    if (military) {
      ...
      const target = targets.find(other => {
        // Reserves remain a strategic threat, even when only a contingent fights.
        if (protectedIds.has(other.factionId) || (strengthByCell.get(other.cell) ?? 0) > army.strength || townByCell.has(other.cell)) return false;
        if (wars.has(other.factionId)) {
          const preview = getMovementQuery(view, army.id, other.cell).preview;
          return preview?.action === 'attack' && preview.canMoveNow;
        }
        // Preserve the historical declaration policy. A new contingent still
        // waits for post-declaration canonical legality before issuing its attack.
      ...
      if (target && plans.length <= 126 && (!minor || wars.has(target.factionId))) {
        if (!wars.has(target.factionId)) {
          ...
          // New contingent attacks need a real post-declaration movement quote;
          // do not invent a war in the observation to bypass its current blocker.
          if (target.battleDefense) return { commands: plans, reasons };
        }
    ...
    // gain still invokes the bounded frontier search, never a center magnet.
