...
test('magic registries describe actual paid research, personal paths and bounded battle effects', () => {
  expect(validateContent()).toMatchObject({ arcaneDiscoveries: 2, battleSpells: 2, magicPaths: 2, innateBattleAbilities: 1 });
  ...
  expect(BATTLE_SPELLS.map(item => item.kind)).toEqual(['damage', 'ward']);
  expect(BATTLE_SPELLS.every(item => item.strainCost > 0 && item.strainCost <= MAX_CASTER_STRAIN && item.uses === 2)).toBe(true);
