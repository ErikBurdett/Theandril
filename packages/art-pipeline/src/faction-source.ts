...
import { FACTION_ART_FAMILIES, FACTION_ART_ROLES, factionArtId, type FactionArtFamily, type FactionArtRole } from './faction-art';
...
export const factionExpansionBatchSchema = z.object({
  ...
  for (const source of batch.sources) {
    if (FACTION_SOURCE_KINDS[source.family] !== 'batch' || source.id !== `${source.role}.${source.family}`) issue(`Unregistered expansion identity ${source.id}`);
