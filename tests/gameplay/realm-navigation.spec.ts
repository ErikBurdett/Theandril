...
test('unified registry finds the hundredth army and fortieth town without rendering the whole empire or changing orders', async ({ page }, testInfo) => {
  ...
  // A page.goto here would accidentally conceal stale registry presentation state.
  await page.locator('input[type=file]').setInputFiles({ name: 'same-ids-reimport.theandril', mimeType: 'application/gzip', buffer: Buffer.from(await exportSave(serializeGame(game))) });
  ...
  // Merely browsing towns does not change the selected army. A same-ID import
  // must reset the active registry too, even though no selection effect fires.
  ...
  expect(await page.evaluate(() => window.__THEANDRIL__!.getSelection())).toEqual(initialSelection);
  await page.locator('input[type=file]').setInputFiles({ name: 'same-ids-wrong-tab.theandril', mimeType: 'application/gzip', buffer: Buffer.from(await exportSave(serializeGame(game))) });
  ...
  await page.getByRole('button', { name: 'Show selected orders', exact: true }).click();
  await expect(page.getByRole('complementary', { name: 'Selected entity orders' })).toBeFocused();
