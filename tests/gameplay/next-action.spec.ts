import { closeManagement, openRegistry, openSelectedOrders } from './ui-navigation';
...
test('paused routes need attention while standing journeys, stationary missions and embarked troops are skipped', async ({ page }) => {
  ...
  await expect(page.getByTestId('next-action-counts')).toContainText('1 needing orders');
  ...
  await expect(page.getByTestId('next-action-counts')).toContainText('2 needing orders');
  await nextArmy(page).click();
