...
export class GroupPostingRequests {
  private pending?: { id: number; resolve: (value: GroupPostingResult[]) => void; reject: (error: Error) => void };
  get busy(): boolean { return Boolean(this.pending); }
  has(id: number): boolean { return this.pending?.id === id; }
  enqueue(id: number): Promise<GroupPostingResult[]> {
  ...
  finish(id: number, result: GroupPostingResult[] | Error): void {
