...
function CharacterSheet({ character, view, busy, initialArmyId, issue, locate }: { character: CharacterView; view: Observation; busy: boolean; initialArmyId?: string; issue: Issue; locate: (character: CharacterView) => void }) {
  ...
  return <section className="character-sheet" aria-label="Selected character" data-testid="character-sheet">
    ...
    <dl className="character-values"><div><dt>Condition</dt><dd>{character.status}{character.woundedTurns ? ` · ${character.woundedTurns} turns` : ''}</dd></div><div><dt>Experience</dt><dd>{character.experience} · rank {character.rank}</dd></div><div><dt>Specialization</dt><dd>{character.skillId ? skillName(character.skillId) : 'Not chosen'}</dd></div></dl>
    ...
    {!character.dead && <>
      <h4>Army attachment</h4><p className="field-help">Appointments and transfers require the same hex. An army has one marshal and up to two agents. Review the available destination and any restriction before changing an assignment.</p>
      ...
      {character.promotions.length > 0 && <><h4>Experience & specialization</h4><p className="field-help">A specialization is a permanent choice. Only the character’s earned experience can purchase it.</p>{character.promotions.map(option => <article key={option.skillId} className="character-promotion"><strong>{option.name}</strong><p>{option.description}</p><p>{option.experienceCost} experience</p>{option.blocker && <p className="character-blocker">{option.blocker}</p>}<button disabled={busy || !option.canPromote} aria-label={`Promote ${option.name}`} onClick={() => issue({ type: 'promoteCharacter', factionId: view.factionId, characterId: character.id, skillId: option.skillId })}>Choose specialization</button></article>)}</>}
    </>}
  </section>;
}
