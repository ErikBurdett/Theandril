...
it('ships only traceable local imagery under a three MiB journal budget', () => {
  ...
  expect(assets.map(asset => asset.id)).toEqual(['campaign', 'battle', 'cultures', 'archipelago', 'technical-ledger', 'transport-landing', 'fleet-provisions-saved-voyage', 'fleet-provisions-exhausted-narrow', 'fleet-provisions-refilled-narrow', 'group-postings-desktop', 'group-postings-narrow', 'group-charters-narrow']);
  let bytes = 0;
  for (const asset of assets) {
    ...
    if (asset.id.startsWith('fleet-provisions-') || asset.id.startsWith('group-postings-') || asset.id === 'group-charters-narrow') {
