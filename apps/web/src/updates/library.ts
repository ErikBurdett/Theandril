...
export const libraryRevision = '0d26fa3c34ac89164637f515e95671420400a841';
...
export const scopeLedger: ScopeEntry[] = [
  ...
  { id: 'campaign-integrity', state: 'Current / partial', title: 'Gates E & F · Save integrity and combat', description: 'Rules32 retains campaign groups with exact rules31 checkpoints and mixed-history saves and exports. A benchmark exposed a pre-existing sixty-four-realm research-save limit; current saves now support it while historical schemas remain frozen. Campaign-safety and fleet-store evidence remains historical; whole save, combat and release gates remain open.', path: 'docs/development/2026-09-25-selection-groups/integration-review.md' },
  ...
  { id: 'ai-integration', state: 'Open gate', title: 'Gate C & integration · Remaining campaign proof', description: 'The saved-groups checkpoint passes 1,941 headless tests across 243 files, 20 affected Chromium journeys and one separate built-production group journey. Rules/save32 adds organizational metadata and repairs the supported save limit; no campaign-resolution rule or AI policy changes. Fleets retain their earlier observed supply and staging behavior, with recorded attrition and Standard/Long pacing limits; these are historical results, not new measurements. M0 and overall integration remain open.', path: 'docs/development/2026-09-25-selection-groups/README.md' },
  { id: 'empire-delegation', state: 'Current / partial', title: 'Gate I · Orders across a large realm', description: 'Up to 24 named campaign groups retain army/hearth selections, with 128 members each and explicit Recall before orders. Existing charter and posting batches preserve individual overrides and refusals. Personal focus/ceiling templates remain browser-local and excluded from campaign exports; saved selection groups travel with the campaign. Broader order templates, theaters and combined mature-empire acceptance remain open.', path: 'docs/development/2026-09-25-selection-groups/README.md' },
...
export const library: Evidence[] = [
  ...
  { label: 'Saved groups architecture', path: 'docs/architecture/0042-selection-groups.md', note: 'Campaign-owned selection groups, explicit recall, loss reconciliation, bounded metadata and historical saves.' },
  { label: 'Charter templates architecture', path: 'docs/architecture/0041-charter-templates.md', note: 'Browser-local named policies, explicit recall and assignment, bounded preferences and visible storage failures.' },
  { label: 'Group charters architecture', path: 'docs/architecture/0040-group-charters.md', note: 'Shared policy controls, existing production precedence, per-work budgets and one final worker response.' },
  { label: 'Group postings architecture', path: 'docs/architecture/0039-group-postings.md', note: 'Bounded ordinary-command batches, per-army results, interrupted recording and transfer measurements.' },
...
export const roadmapSnapshot = { date: '2026-09-26', revision: libraryRevision, rules: 32 };
...
const groupEvidence: Evidence = { label: 'Group postings verification', path: 'docs/development/2026-09-23-deploy-and-group-postings/README.md', note: '1,876 headless tests, seven affected Chromium journeys and 27 production Pages checks at the implementation checkpoint; authored scale, partial refusal and recovery evidence.' };

const charterEvidence: Evidence = { label: 'Group charters verification', path: 'docs/development/2026-09-24-group-charters/README.md', note: '1,889 tests across 235 files, 12 affected Chromium journeys and one new built-production charter journey; authored scale, queues, partial refusal and saved recovery.' };

const templateEvidence: Evidence = { label: 'Charter templates verification', path: 'docs/development/2026-09-25-charter-templates/README.md', note: '1,902 tests across 237 files, 16 affected Chromium journeys and one separate built-production template journey; browser-local policies, explicit application and storage recovery.' };

const selectionEvidence: Evidence = { label: 'Saved groups verification', path: 'docs/development/2026-09-25-selection-groups/README.md', note: '1,941 tests across 243 files, 20 affected Chromium journeys and one separate built-production group journey; saved membership, exact history, bounded metadata and sixty-four-realm save repair.' };
...
export const roadmapItems: RoadmapItem[] = [
  ...
  },
  {
    ...
    summary: 'Searchable registries apply policies to up to 128 owned hearths or post up to 128 land armies ashore. Named campaign groups recall selections; a separate personal browser library recalls charter policies. Orders remain explicit. ACT-32 and wider M3 coordination remain in progress.',
    ...
    remaining: ['Extend charters and postings with broader governor decisions, bounded theaters, patrol/escort roles, saved reusable army order templates, production sequences and coordinated orders, keeping inspectable blockers and clear overrides.', 'Complete broader governor decisions, onboarding, accessible keyboard/tooltip workflows, durable settings and critical canvas alternatives for all major systems; group assignment, saved selections and personal charter templates do not complete them.', 'Prove combined delegation and override in representative mature campaigns with 100+ armies and many settlements; the authored group-order fixture is a bounded checkpoint. Profile sustained query transfers and virtualize large registries where needed.'],
    gates: ['I'], evidence: [selectionEvidence, templateEvidence, charterEvidence, groupEvidence, statusEvidence, gateEvidence],
  },
  {
  ...
  },
  {
    ...
    delivered: ['Packed map transfers, bounded land/development queries and measured Huge camera workloads have retained evidence.', 'Bounded group reconciliation is measured at 147,456/196,608 membership references across forty-eight/sixty-four authored realms, with fifteen samples and strict saved roundtrips. JSON sizes and isolated timings do not establish full-turn, worker or renderer budgets.'],
    ...
    gates: ['D', 'K'], evidence: [selectionEvidence, statusEvidence, { label: 'Performance record', path: 'docs/PERFORMANCE.md', note: 'Measured workloads and their limits; results must match the candidate release revision.' }, gateEvidence],
  },
  {
    ...
    delivered: ['Digest-verified publication and reference discovery refuse unsafe writes while preserving the last readable generation.', 'Rules32 preserves campaign groups, their separate ID counter and exact rules31 history. Supported sixty-four-realm campaigns now roundtrip their research register; frozen historical schemas retain forty-eight-row limits and refuse incompatible exports.'],
    ...
    gates: ['E', 'K'], evidence: [selectionEvidence, reviewEvidence, { label: 'Storage report and limits', path: 'docs/development/review-fix-2/persistence-REPORT.md', note: 'Scoped correctness approval, O(prefix bytes) publication, retained samples and unverified durability/performance cases.' }, gateEvidence],
  },
  {
  ...
  },
  {
    ...
    delivered: ['Headless campaigns, battle-boundary saves, history replay and targeted production journeys have checkpoint-specific evidence.', 'The saved-groups implementation checkpoint passes 1,941 headless tests across 243 files with four local workers, 20 affected Chromium gameplay journeys, one separate built-production group journey and the Pages-subpath build. These overlapping execution scopes are not added together; they precede publication of its dispatch and its separate publication checks.', 'The dated 21 September evidence retains its original 25 gameplay/27 Pages results and former Epic timing failure; it is historical context, not the latest full-suite result.'],
    ...
    gates: ['A', 'B', 'C', 'E', 'F', 'F2', 'G', 'H'], evidence: [selectionEvidence, templateEvidence, charterEvidence, groupEvidence, fleetEvidence, campaignEvidence, reviewEvidence, gateEvidence],
  },
  {
