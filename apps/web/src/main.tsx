...
import { GENERATOR_VERSION, MAP_TYPES, RECOMMENDED_CITY_STATES, RECOMMENDED_FACTION_COUNTS, mapDimensions, type MapSize, type WorldLayout } from '@theandril/mapgen';
...
const WORLD_SIZES: readonly (readonly [MapSize, string])[] = [['tiny', 'Tiny'], ['small', 'Small'], ['standard', 'Standard'], ['huge', 'Large'], ['legendary', 'Huge']];
...
function App() {
  ...
  const [factionCount, setFactionCount] = useState(String(RECOMMENDED_FACTION_COUNTS.standard));
  const [cityStateCount, setCityStateCount] = useState(String(RECOMMENDED_CITY_STATES.standard));
  const [factionDefinitionId, setFactionDefinitionId] = useState('faction.ashen_compact');
  const changeWorldSize = (next: MapSize) => { setSize(next); setFactionCount(String(RECOMMENDED_FACTION_COUNTS[next])); setCityStateCount(String(RECOMMENDED_CITY_STATES[next])); };
  ...
  const [layout, setLayout] = useState<WorldLayout>('continents');
  ...
  const begin = (event: FormEvent) => {
    ...
    setGenerating(true); send({ type: 'new', seed: value, size, mode: newMode, pace, factionCount: count, cityStateCount: Math.max(0, Math.min(24, Number(cityStateCount) || 0)), factionDefinitionId, layout });
  };
  ...
  // Rules 27 supply is a real constraint, so the ground a realm can feed is drawn
  // under the map by default; it can be turned off from the map guide.
  useEffect(() => { renderer.current?.setSupplyRange(showSupply && observation ? observation.suppliedCells : []); }, [observation, showSupply]);
    ...
    {!observation || showSetup ? <main className="landing">
      ...
      <section className="setup panel"><span className="eyebrow">A chronicle begins</span><h2>Establish your campaign</h2><div className="faction-card"><FactionArt contentId="ui.crest" definitionId={factionDefinitionId} label="Selected culture crest" decorative/><div><h3>{FACTIONS.find(faction => faction.id === factionDefinitionId)?.name}</h3><p>Your chosen player seat</p></div></div>
        <form onSubmit={begin}>
          ...
          <label>World size<select value={size} onChange={event => changeWorldSize(event.target.value as MapSize)}>{WORLD_SIZES.map(([value, label]) => { const { width, height } = mapDimensions(value, GENERATOR_VERSION); return <option key={value} value={value}>{label} · {(width * height).toLocaleString('en-US')} hexes · {RECOMMENDED_FACTION_COUNTS[value]} realms</option>; })}</select></label>
          <label>Faction count<input type="number" min={2} max={42} step={1} required value={factionCount} aria-describedby="faction-density-help" onChange={event => setFactionCount(event.target.value)}/></label>
          <label>City-states<input type="number" min={0} max={24} step={1} required value={cityStateCount} aria-describedby="city-state-help" onChange={event => setCityStateCount(event.target.value)}/></label>
          <p id="city-state-help" className="field-help" data-testid="city-state-help">Independent single-city powers that settle the land between realms and can be conquered like any neighbour. Recommended here: {RECOMMENDED_CITY_STATES[size]}. They borrow a culture's art under their own banner colour.</p>
          ...
          <label>Map type<select value={layout} aria-describedby="map-type-help" onChange={event => setLayout(event.target.value as WorldLayout)}>{MAP_TYPES.map(type => <option key={type.id} value={type.id}>{type.name}</option>)}</select></label>
          <p id="map-type-help" className="field-help">{MAP_TYPES.find(type => type.id === layout)?.description}</p>
          <label>Campaign pace<select value={pace} onChange={event => setPace(event.target.value as CampaignPace)}><option value="short">Short · test/skirmish</option><option value="standard">Standard · about 200 turns</option><option value="long">Long · about 300 turns</option><option value="epic">Epic · about 350–400 turns</option></select></label>
          ...
          <p className="field-help">The same seed, size, map type and faction count create the same world within a generator version. Larger maps need more realms to create nearby rivals; terrain can still separate them.</p>
