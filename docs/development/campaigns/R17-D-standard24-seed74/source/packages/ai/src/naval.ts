...
export function oceanScoutReserve(view: Observation): number {
  ...
  const cells = new Map(view.cells.map(cell => [cell.cell, cell])), sea = createSeaKnowledge(view, cells);
  ...
  const secondOutlet = view.growth && sparseContactNeeded(view) && view.factions.length === 1
    && towns.filter(town => town.buildings.includes('building.harbor')).length >= 2 ? preferredPort(view, cells, sea) : undefined;
  const water = secondOutlet ? portWaters(view, secondOutlet, cells) : [];
...
export function coastalFoundingSite(view: Observation, army: ArmyView): number | null {
  ...
  if (view.growth && sparseContactNeeded(view) && view.factions.length === 1) {
    ...
    if (shores.length === 1 && !underway) {
      ...
      const candidates = view.cells.filter(cell => isPassable(cell.terrain) && !cell.settlementId && !occupied.has(cell.cell)
        ...
        && coastalWaters(view, cell.cell, cells).some(next => isSea(cells.get(next)) && relationToPort(water, next, sea) !== 'connected'))
        .sort((a, b) => hexDistance(army.cell, a.cell, view.width) - hexDistance(army.cell, b.cell, view.width) || a.cell - b.cell).slice(0, 4);
...
export function planNaval(view: Observation, coinBudget: number, options: NavalPlanOptions = {}): NavalPlan {
  ...
  const distantContact = view.width * view.height / Math.max(1, view.factionCount) > 12_000 && view.factions.length < view.factionCount;
