...
export function foundingCoinCost(existingCount: number, version = 16): number { return version >= 16 ? safe(existingCount * (12 + 2 * existingCount)) : 0; }
...
export function settlementCivicUpkeep(population: number, claims: number, townCount: number, version = 16): CivicUpkeep {
  ...
  const territory = version >= 16 ? safe(Math.ceil(Math.max(0, claims - 7) / 12)) : 0;
  const administration = version >= 16 ? safe(Math.floor(Math.max(0, townCount - 1) / 3)) : 0;
...
export function getGrowthObservation(state: GameState, factionId: string, towns?: readonly Settlement[]): GrowthObservation | undefined {
  if (rulesVersion(state) < 16) return undefined;
  const owned = towns ?? Object.values(state.settlements).filter(town => town.factionId === factionId), count = owned.length;
  const owner = state.factions.find(faction => faction.id === factionId), coinCost = foundingCoinCost(count);
  const settlements = owned.map(town => ({ settlementId: town.id, foodRequired: settlementGrowthFood(town.population), foodConsumption: settlementFoodConsumption(town.population), civicUpkeep: settlementCivicUpkeep(town.population, state.land.settlements[town.id]?.claimed.length ?? 1, count) }));
  const administrationBefore = settlementCivicUpkeep(1, 7, count).administration, next = settlementCivicUpkeep(1, 7, count + 1);
