...
for (const item of previewCorpus()) for (const mode of ['full', 'preview'] as const) {
  const invoke = (api: typeof after) => mode === 'full' ? api.getMovementQuery(item.view, item.armyId, item.target, { append: item.append }) : api.getMovementPreview(item.view, item.armyId, item.target, { append: item.append });
  ...
  for (let sample = -5; sample < 40; sample++) for (const side of sample % 2 ? ['before', 'after'] as const : ['after', 'before'] as const) {
    const started = performance.now();
    const result = invoke(side === 'before' ? before : after);
