...
- Name: household-attention.spec.ts >> R07 growing queued hearth exposes exact labor attention and preserves manual assignments until a real order
- Location: tests/gameplay/household-attention.spec.ts:15:1
...
Locator: getByRole('button', { name: 'Next settlement with unassigned households', exact: true })
Expected: disabled
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Expect "toBeDisabled" getByRole('button', { name: 'Next settlement with unassigned households', exact: true }) with timeout 5000ms
  - waiting for getByRole('button', { name: 'Next settlement with unassigned households', exact: true })
...
- contentinfo:
  - text: Selected army
  - strong: Wayfinder
  - text: 1 / 12 formations · 20 strength · 5 movement
  - button "Show selected orders"
  - button "Show on map"
  - region "Next-action navigation":
    - paragraph: 1 needing orders · 0 idle settlements
  ...
  - status: "Imported campaign. Partial archive: earlier campaign history was not recorded in this save."
- button "Art Lab"
...
> 17 |   await expect(review(page)).toBeDisabled();
  ...
  70 |     await popup.screenshot({ path: info.outputPath('household-review-390.png') });
  71 |     await popup.getByRole('button', { name: 'Select tiles', exact: true }).click();
  72 |     await popup.locator('.land-tile-list button').filter({ hasText: /Owned/ }).first().click();
  73 |     await popup.getByRole('button', { name: 'Assign worker', exact: true }).click();
  74 |     await expect(page.getByTestId('household-counts')).toHaveText('Labor: 0 unassigned households · 0 settlements');
  75 |     const after = await page.evaluate(() => window.__THEANDRIL__!.getSummary()!);
  76 |     expect(after.treasury).toBe(before.view.treasury);
  77 |     for (const land of before.view.land.settlements) {
  78 |       const worked = after.land.settlements.find(item => item.settlementId === land.settlementId)!.worked;
  79 |       if (land.settlementId !== target.id) expect(worked).toEqual(land.worked);
  80 |       else expect(worked).toEqual(expect.arrayContaining(land.worked));
  81 |     }
