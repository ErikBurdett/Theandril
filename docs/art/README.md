...
The six authored cultures each have 15 qualified land roles; [visual bible](THEANDRIL_ART_BIBLE.md) and [faction coverage](FACTION_ASSET_CATALOG.md) define the actual consumers. The first four use retained sheets: exact source/revision prompts live in `assets/art/source/faction-generation-prompts.json` and `faction-revisions.json`. The dedicated `faction-index.json` never replaces the generic `foundation-index.json`.
