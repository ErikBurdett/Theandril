...
Published 2026-09-09: **545 approved assets / 1,427 frames**. This includes [432 culture assets](FACTION_ASSET_CATALOG.md), 24 biome variants, the preceding improvement/civic originals, sixteen new resource/works originals and [13 shared articulated battle roles](reviews/BATTLE_UNIT_ANIMATION.md) with 832 frames / 130 directional clips. All use the 64-color master palette and exact hard alpha. The foundation remains 2,628,301 PNG bytes / 16 MiB decoded, SHA-256 `e32c73be1b8789f8c554eea7c6b7204d40fae20e3347be36ccbd76de8a9740de`. Map-works uses 89,796 PNG bytes / 1 MiB. Three deferred battle pages use 26,100 / 626,544 / 779,449 PNG bytes and 4 / 16 / 16 MiB. Total PNG download is 4,150,190 bytes; map atlas residency remains 17 MiB and all five pages total 53 MiB. These are atlas figures, not total presentation memory. The retained [24-culture galleries](FACTION_ASSET_CATALOG.md#current-in-game-review) and [earlier effects review](reviews/slice23-battlefield.md) remain scoped evidence for their unchanged art. New native and runtime acceptance is distinguished in [art status](ART_IMPLEMENTATION_STATUS.md).
...
## Hearth development additions

Five researched improvements and five civic buildings now have independent
reviewed64×64 originals with `(32,48)` pivots and static southeast frames:
`improvement.spring_garden`, `improvement.polder`, `improvement.grove_archive`,
`improvement.oreworks`, `improvement.tide_observatory`, `building.granary`,
`building.workshop`, `building.market`, `building.archive`, `building.harbor`.
[Sources, exact approvals, native and in-game review](reviews/HEARTH_IMPROVEMENTS.md)
and [growing hearth evidence](reviews/slice26/README.md) are retained.

The four specialist troop definitions deliberately share existing faction
scout/pike/cavalry silhouettes via `unitArtRole`; these are additional gameplay
roles, not additional approved sprite families.
