// Audit copy of scripts/benchmark-chronicle.ts; only persistence of evidence added.
import { writeFileSync } from 'node:fs';
import { gzipSync } from 'node:zlib';
import { performance } from 'node:perf_hooks';
import { cpus } from 'node:os';
import { createGame, getObservation, stateHash, serializeGame, type GameCommand } from '@theandril/sim';
...
for (const { size, pace, factions, limit } of cases) {
  ...
  const capturePath = `/home/telephoneheater/Work/Theandril/docs/hermes-analysis/qa/captured-${size}-${pace}-${seed}.json.gz`;
  writeFileSync(capturePath, gzipSync(JSON.stringify({ snapshot: serializeGame(state), archive })));
  console.error('CAPTURE_META ' + JSON.stringify({ capturePath, seed, size, pace, factions, turn: state.turn, victory: state.victory, orders: archive.records.length, rejected, hash: stateHash(state), rawSnapshotBytes: Buffer.byteLength(serializeGame(state)), recordedBattles: archive.records.reduce((sum, record) => sum + record.battles.length, 0) }));
