...
export function planStrategy(view: Observation, strategy: 'B' | 'C'): AiPlan {
  ...
  const quote = (settlementId: string, itemId: string) => view.productionOptions.find(q => q.settlementId === settlementId && q.itemId === itemId && q.canQueue && q.coinCost <= spendable);
  if (strategy === 'B') {
    ...
    if (view.turn % 3 !== 0 && military < towns.length * 6 + 2) {
      const roster = ['unit.guard', 'unit.pike', 'unit.arbalester', 'unit.outrider', 'unit.skirmisher'];
    ...
    for (const itemId of ['building.archive', 'building.market', 'building.workshop', 'building.cellar']) for (const town of towns.filter(t => !t.queue.length)) {
