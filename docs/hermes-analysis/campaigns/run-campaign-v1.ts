/** Audit-only runner: canonical commands, observation-only clients, full exact replay. */
import assert from 'node:assert/strict';
import { createHash } from 'node:crypto';
import { appendFileSync, mkdirSync, readFileSync, writeFileSync, createReadStream } from 'node:fs';
...
const settings: NewGameOptions = { seed: Number(args.seed ?? 74), size: (args.size ?? 'tiny') as NewGameOptions['size'], factionCount: Number(args.factions ?? 4), pace: (args.pace ?? 'standard') as NewGameOptions['pace'], generatorVersion: 7, rosterVersion: 4, rulesVersion: 16 };
...
assert.equal(Number(SAVE_VERSION), 16); assert.equal(Number(GENERATOR_VERSION), 7);
const out = resolve('docs/hermes-analysis/campaigns/runs', name); mkdirSync(out, { recursive: true });
...
const custom = strategy === 'D' ? null : await import('./strategies.js');
...
try {
  while (rounds < maxRounds && !game.victory) {
    ...
    if (rounds === midpointRound) {
      const saved = serializeGame(game); mirror = deserializeGame(saved); assert.equal(serializeGame(mirror), saved);
