...
FOOT_ROLES = ('colonist', 'scout', 'guard', 'spearman', 'heavy_infantry', 'skirmisher', 'arbalester', 'halberdier')
MOUNTED_ROLES = ('cavalry', 'lancer')
NAVAL_ROLES = ('transport', 'coastal_warship', 'ocean_warship')
...
def build(factory_root, role, version, states, directions, execute=False, blender=None):
    if role not in FOOT_ROLES + MOUNTED_ROLES + NAVAL_ROLES:
        raise ValueError('Role is not yet authored in this factory revision.')
    ...
    size, pivot = (64, [32, 56]) if role in FOOT_ROLES else (96, [48, 80])
    clips = {({'walk': 'sail', 'attack': 'fire', 'death': 'sink'}.get(state, state) if role in NAVAL_ROLES else state): CLIPS[state] for state in states}
    profile = {'schemaVersion': 1, 'id': 'battle.unit.' + role, 'role': role, 'workflow': 'pixel_2d', 'palette': palette,
               'textures': {'size': 8, 'filter': 'nearest'}, 'sprites': {'size': [size, size], 'anchor': pivot},
               'camera': {'position': [0, -6, 3.5], 'orthoScale': 4.8 if role in NAVAL_ROLES else 3.9 if size == 96 else 2.6}, 'directions': directions,
               'clips': clips,
    ...
    recipe_name = 'guard.py' if role == 'guard' else 'mounted.py' if role in MOUNTED_ROLES else 'naval.py' if role in NAVAL_ROLES else 'foot.py'
    for origin, name in [(HERE / recipe_name, 'recipe.py'), (HERE / 'guard.py', 'guard_base.py'), (HERE / 'driver.py', 'driver.py'), (HERE / 'build.py', 'build.py'),
    ...
    for frame in settings['frames']:
        ...
        quantize(output / frame['rawPath'], output / name, [size, size], palette['colors'], .5)
        frame['path'], frame['sha256'] = name, sha(output / name)
        images.append(read_png(output / name))
    validation = validate(images, settings['frames'], palette['colors'], size, pivot, profile['clips'], directions)
    ...
    manifest = {'schemaVersion': 1, 'id': profile['id'], 'contentId': 'unit.' + role, 'version': version,
                ...
                'nativeResolution': {'width': size, 'height': size}, 'pivot': pivot, 'paletteHash': sha(output / 'palette.json'),
    ...
    return {**plan, 'dryRun': False, 'manifest': str(output / 'manifest.json'), 'validation': {'ok': validation['ok'], 'frames': len(validation['frames']), 'errors': validation['errors']}}
