...
REPO = HERE.parents[2]
EFFECTS = {'melee': 'effect.battle_melee', 'projectile': 'effect.battle_projectile',
           'ember': 'effect.battle_ember', 'ward': 'effect.battle_ward',
           'rally': 'effect.battle_rally', 'waykeeper': 'character.waykeeper'}
...
def validate_frames(images, colors, anchors, padding=3, expected_anchor=(32, 32)):
    ...
    anchor_error = max((abs(anchor[axis] - expected_anchor[i]) for anchor in anchors for i, axis in enumerate(('x', 'y'))), default=0)
    ...
    if len(anchors) != len(images) or anchor_error > .0001 or anchor_drift > .0001:
        errors.append('Fixed declared anchor must retain its projection in every frame.')
...
def build(factory_root, effect, version, execute=False, blender=None):
    ...
    for origin, name in [(recipe, 'recipe.py'), (HERE / 'driver.py', 'driver.py'), (HERE / 'build.py', 'build.py'), (HERE / 'battle_authoring.py', 'battle_authoring.py'), (profile_path, 'authored-profile.json'), (REPO / 'assets/palettes/theandril-master.json', 'palette.json')]:
    ...
    validation = validate_frames(images, profile['palette']['colors'], settings['anchorProof'], expected_anchor=profile['sprites']['anchor'])
    ...
    manifest = {'schemaVersion': 1, 'id': EFFECTS[effect], 'version': version,
                ...
                'nativeResolution': {'width': 64, 'height': 64}, 'pivot': dict(zip(('x', 'y'), profile['sprites']['anchor'])),
