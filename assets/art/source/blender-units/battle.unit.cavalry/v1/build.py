...
def build(factory_root, role, version, states, directions, execute=False, blender=None):
    if role not in FOOT_ROLES + MOUNTED_ROLES:
    ...
    size, pivot = (96, [48, 80]) if role in MOUNTED_ROLES else (64, [32, 56])
    ...
    recipe_name = 'guard.py' if role == 'guard' else 'mounted.py' if role in MOUNTED_ROLES else 'foot.py'
