...
def points(state, index, count):
    ...
    lean = .025 * idle + hit * .22 + fall * 1.5
    pelvis = v((0, -.035 * attack + .04 * hit, .94 - .025 * math.cos(phase * 2) * walk - .74 * fall))
    ...
    for side, label in [(-1, 'L'), (1, 'R')]:
        ...
        if side < 0:
            ...
            if state == 'attack':
                positions = [(.34, -.13, -.25), (.40, .08, -.05), (.32, .12, .15), (.28, -.32, .16), (.06, -.42, -.16), (.15, -.31, -.28), (.32, -.18, -.28), (.34, -.13, -.25)]
        ...
        if ROLE in ('scout', 'arbalester'):
            if side < 0:
                wrist = chest + v((-.20, -.34 - .10 * attack, -.08))
                hand_direction = v((0, -.12, .01))
            elif state == 'attack':
                draw = [0, .35, .75, 1, .18, .1, .05, 0][index]
                wrist = chest + v((.12, -.35 + .34 * draw, -.05 + .06 * draw))
                hand_direction = v((0, -.12, .01))
        if ROLE in ('spearman', 'halberdier') and state == 'attack':
            thrust = [0, -.04, -.10, .13, .24, .16, .04, 0][index]
            wrist = chest + v((side * .22, -.22 - thrust, -.18))
            hand_direction = v((0, -.115, .034))
        if fall:
            wrist = wrist.lerp(v((side * (.25 if ROLE in ('spearman', 'halberdier', 'colonist', 'skirmisher') else .35), .34, .18)), fall)
            # Lay both pieces of equipment on the ground as the body collapses.
            # The shield rotates around the wrist; it never sinks upright below
            # the floor. The sword points into the same fixed frame union.
            end_hand = (-.05, .01, .12) if side < 0 else (.025, .12, .01)
            hand_direction = hand_direction.lerp(v(end_hand), fall)
            if side > 0 and ROLE in ('spearman', 'halberdier'):
                wrist.z = max(wrist.z, .95 * hand_direction.normalized().z + .18)
            if side > 0 and ROLE == 'colonist':
                wrist.z = max(wrist.z, .78 * hand_direction.normalized().z + .15)
    ...
    if fall:
        # The backward fall carries the pelvis away from the planted origin.
        # Keep one shared ground pivot and camera for every action; never crop
        # or fit a collapsed pose independently.
        displacement = v((0, .30 * fall, 0))
        result = {name: (start + displacement, end + displacement) if name != 'root' else (start, end)
...
def build(ctx):
    global ROLE
    ROLE = ctx.profile.get('role', 'guard')
