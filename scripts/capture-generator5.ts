...
// Historical capture tool: deliberately refuses to regenerate evidence after v6.
assert.equal(GENERATOR_VERSION, 5); assert.equal(SAVE_VERSION, 12); assert.equal(CONTENT_HASH, '3c54fb02');
...
for (const layout of ['continents', 'islands', 'archipelago'] as const) {
  const game = createGame({ seed: 74, size: 'tiny', factionCount: 4, layout, generatorVersion: 5, pace: 'epic' });
...
const packed = { capturedBefore: 'generator6 biome classifier; current b17900d-based schema12/generator5 implementation, ordinary orders only', hashes: Object.fromEntries(cases.map(item => [item.name, item.hash])), payload: gzipSync(JSON.stringify(payload), { level: 9 }).toString('base64') };
writeFileSync('packages/chronicle/src/fixtures/v12-generator5-history.json', JSON.stringify(packed, null, 2) + '\n');
